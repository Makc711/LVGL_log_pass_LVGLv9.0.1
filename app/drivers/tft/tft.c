...
typedef lv_color16_t color_depth_t;
#elif LV_COLOR_DEPTH == 24
typedef uint32_t uintpixel_t;
typedef lv_color_t color_depth_t;
#elif LV_COLOR_DEPTH == 32
typedef uint32_t uintpixel_t;
typedef lv_color32_t color_depth_t;
#endif
...
//static uintpixel_t my_fb[TFT_HOR_RES * TFT_VER_RES] SDRAM_DATA;
...
static const color_depth_t* buf_to_flush;
...
/**
 * @brief Initialize your display here
 */
void tft_init(void)
{
  ...
  static color_depth_t buf1[TFT_HOR_RES * (TFT_VER_RES / 4)]; /*At least 10 lines for quick draws*/
  static color_depth_t buf2[TFT_HOR_RES * (TFT_VER_RES / 4)]; /*At least 10 lines for quick draws*/
 ...
 * You can use DMA or any hardware acceleration to do this operation in the background*/
static void flush_cb(lv_display_t* disp, const lv_area_t* area, uint8_t* px_map)
{
  ...
  const int32_t act_y1 = (y1 < 0) ? 0 : y1;
  const int32_t act_x2 = (x2 > TFT_HOR_RES - 1) ? TFT_HOR_RES - 1 : x2;
  const int32_t act_y2 = (y2 > TFT_VER_RES - 1) ? TFT_VER_RES - 1 : y2;
  ...
  y2_fill = act_y2;
  y_fill_act = act_y1;
  buf_to_flush = (color_depth_t*)px_map;
