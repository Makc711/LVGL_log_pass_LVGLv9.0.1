...
void lv_demo_widgets(void)
{
    ...
    lv_obj_add_event_cb(tabview, tabview_delete_event_cb, LV_EVENT_DELETE, NULL);

    lv_obj_set_style_text_font(lv_screen_active(), font_normal, 0);

    lv_obj_t * t1 = lv_tabview_add_tab(tabview, "Profile");
    lv_obj_t * t2 = lv_tabview_add_tab(tabview, "Analytics");
    lv_obj_t * t3 = lv_tabview_add_tab(tabview, "Shop");
...
static void color_changer_create(lv_obj_t * parent)
{
    ...
    for(uint32_t i = 0; palette[i] != _LV_PALETTE_LAST; i++) 
    {
...
static void slideshow_anim_completed_cb(lv_anim_t * a_old)
{
    ...
    lv_tabview_set_active(tabview, tab_id, LV_ANIM_ON);
